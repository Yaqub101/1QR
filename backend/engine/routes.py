...
class ScanBody(BaseModel):
    model_config = ConfigDict(extra="ignore")  # an "activity" in the body is ignored: the station decides
    token: str
    station_id: str
...
def _call(request: Request, principal: Principal, fn, **kwargs) -> dict:
    app = request.app
    try:
        result = fn(app.state.engine, settings=app.state.settings, guard=app.state.venue_guard,
...
def scan(body: ScanBody, request: Request, principal: Principal = Depends(require_user)):
    return _call(request, principal, service.scan, station_id=body.station_id, token=body.token)


@router.post("/search")
def search(body: SearchBody, request: Request, principal: Principal = Depends(require_user)):
    return _call(request, principal, service.search, station_id=body.station_id, prn=body.prn)


@router.post("/confirm")
def confirm(body: ConfirmBody, request: Request, principal: Principal = Depends(require_user)):
    return _call(request, principal, service.confirm, station_id=body.station_id, token=body.token,
