# STADIUM: if the server stops

**Use this sheet when** the Stadium server (`stadium.local`) has stopped: stations say "One moment…" and then "Use backup — call Admin", and it does not come back after one minute.

**What will happen:** the STANDBY laptop (the Stage backup laptop, marked STANDBY) becomes the server in **under 2 minutes**. Nothing that was confirmed is lost: the standby holds a copy that is only a few seconds behind.

**Who does it:** the Admin or the Event IT lead. Operators do not do this.

## Steps

1. **Tell the operators to carry on on paper.** Write the student's name and the time on the printed list. Everything is typed in later.
...
4. **Type this and press Enter:** `./scripts/failover.sh --venue stadium`
...
9. **Stations come back on their own** in about a minute. If a screen shows the sign-in page, sign in again. Nothing else to do.
10. **The big screen (LED)** shows the university holding screen for a moment, then carries on. The Stage operator presses **TAKE OVER** if the screen says another laptop is running the stage.
...
- The IT lead can also bring the Stadium back from the last backup on the backup drive (taken every 5 minutes).
