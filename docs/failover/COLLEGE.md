# COLLEGE: if the server stops

**Use this sheet when** the College server (`college.local`) has stopped: the Registration desks say "One moment…" and then "Use backup — call Admin", and it does not come back after one minute.
...
**Who does it:** the Admin or the Event IT lead. Registration operators do not do this.

## Steps

1. **Tell the desks to carry on on paper.** Write the student's name and the time on the printed list. Everything is typed in later. Students can keep moving; registration is only a record.
...
4. **Type this and press Enter:** `./scripts/failover.sh --venue college`
...
10. **Type in the paper entries** from the printed list, using the PRN search on a Registration desk.
...
- The IT lead can also bring the College back from the last backup on the backup drive (taken every 5 minutes).
