...
| 14 | **Master-freeze checklist** (signed) | [MASTER_FREEZE_CHECKLIST.md](MASTER_FREEZE_CHECKLIST.md) | EXISTS (blank); filled in at the freeze | Its step 5 (tokens and passes) is **BLOCKED** because Phase 4 is not built. |
| 15 | **Rehearsal script and its signed results** | [REHEARSAL_SCRIPT.md](REHEARSAL_SCRIPT.md) | EXISTS (blank); filled in at the rehearsal | Cannot be run until Phase 4 exists. |
| 16 | **QR and pass procedure** (how tokens are generated, how passes are printed and given out, how a QR is reissued) | nowhere | **NOT BUILT** | Phase 4 (`docs/TODO.md`) has not been built: no token generator, no pass PDF, no reissue action. Only the storage exists (`qr_tokens` in `0002_core_schema.py`, and tokens travel in the master pack, `backend/master_pack.py`). The procedure can only be written once it is built. |
...
1. **Phase 4** (tokens, passes, reissue): not built. Blocks the freeze, the rehearsal and the pass procedure.
