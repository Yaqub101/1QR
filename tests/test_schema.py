...
def complete_before(conn, student_id, activity):
    """Complete every activity of the journey that comes before `activity` (named, not counted)."""
    complete_steps(conn, student_id, ACTIVITIES.index(activity))
...
class TestStudentStatus:
    ...
    def test_seating_moves_to_seated_not_queued(self, conn):
        s = new_student(conn)
        complete_before(conn, s, "SEATING")
